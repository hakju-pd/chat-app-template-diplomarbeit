...
- Der **`service_role`-Schlüssel** ist ein Admin-Schlüssel. Er steht NUR im
  Backend (`.env`) und darf NIEMALS ins Frontend oder auf GitHub.
- Die `.env` Datei ist über `.gitignore` vom Hochladen ausgeschlossen.
- Geschützte Routen (Nachricht senden, Raum anlegen) sind bewusst mit einem
  **einfachen `if`** abgesichert (kein Decorator) – siehe `backend/routes.py`.
